...
class Page extends KuriakosPlugin {
    ...
    public function filter_plugin_action_links(array $actions)
    {
        return array_merge(array(
            'settings' => '<a href="#xt-settings">' . esc_html__('Settings', 'kuriakos-plugin') . '</a>',
    ...
    public function filter_plugin_row_meta(array $plugin_meta, string $plugin_file): array
    {
        ...
        $plugin_meta[] = sprintf(
            ...
            esc_html_x('Sponsor', 'verb', 'kuriakos-plugin')
        );
    ...
    {
        if (!defined('KTV_HIDE_SELF')) {
            define('KTV_HIDE_SELF', true);
        }

        foreach (apply_filters('xt/collectors', array(), $this) as $collector) {
            KTV_Collectors::add($collector);
        }

        foreach ((array)glob($this->plugin_path('dispatchers/*KTV_Migration_Settings_Table.php')) as $file) {
            include_once $file;
        }

        foreach (apply_filters('xt/dispatchers', array(), $this) as $dispatcher) {
            KTV_Dispatchers::add($dispatcher);
        }
    }

    public function action_init()
    {
        load_plugin_textdomain('kuriakos-plugin', false, dirname($this->plugin_base()) . '/languages');
    }
    ...
    public static function symlink_warning()
    {
        ...
        /* translators: %s: Symlink file location */
            esc_html__('The symlink at %s is no longer pointing to the correct location. Please remove the symlink, then deactivate and reactivate Query Monitor.', 'kuriakos-plugin'),
    ...
    {
        members_register_cap_group('kuriakos_plugin', array(
            'label' => __('kuriakos', 'kuriakos-plugin'),
            'caps' => array(
                'view_kuriakos_plugin',
            ),
    ...
    {
        members_register_cap('view_kuriakos_plugin', array(
            'label' => _x('View kuriakos Classes', 'Human readable label for the user capability required to view kuriakos Classes.', 'kuriakos-plugin'),
            'group' => 'kuriakos_plugin',
        ));
    }

    public function filter_ure_groups(array $groups)
    {
        $groups['kuriakos_plugin'] = array(
            'caption' => esc_html__('kuriakos', 'kuriakos-plugin'),
            'parent' => 'custom',
            'level' => 2,
        );
    ...
    {
        $caps['view_kuriakos_plugin'] = array(
            'custom',
            'kuriakos_plugin',
        );
